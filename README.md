...
### 商標實體工具 — 工具 1：商品服務名稱初篩工具

上傳商品服務名稱清單表（Excel） → 選擇要檢查的類別頁 → 輸入比對基礎（品牌類型描述） → 設定判斷寬嚴度 → AI 自動比對並在欄位 E 標示符合項目 → 下載標示結果 Excel。
...
- **AI 商標初篩** — 透過 Claude API 根據品牌類型自動判斷商品服務項目的關聯性
...
| anthropic | Claude API 呼叫（Vision OCR、商標初篩） |
| openpyxl | Excel 檔案讀寫（商標初篩工具） |

## 注意事項

- Regex 擷取模式不需要 API，零成本且 100% 準確
- Vision OCR 模式每次擷取約消耗 1 次 Claude API 呼叫
- 商品服務名稱初篩工具每個類別頁約消耗 1～數次 Claude API 呼叫（依項目數量而定）
- 下載間隔預設 1 秒，可自行調整
